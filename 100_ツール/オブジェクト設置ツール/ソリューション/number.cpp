...
{
	"data\\TEXTURE\\Score000.png",	// 背景000のテクスチャ
};
...
void CNumber::SetVtx(void)
{
	...
	int nDigits = m_nDataDigits.Get();		// 桁数
	...
	// テクスチャの座標を設定
	pVtx[0].tex = D3DXVECTOR2(0.1f * (nDigits + 0), 0.0f);
	pVtx[1].tex = D3DXVECTOR2(0.1f * (nDigits + 1), 0.0f);
	pVtx[2].tex = D3DXVECTOR2(0.1f * (nDigits + 0), 1.0f);
	pVtx[3].tex = D3DXVECTOR2(0.1f * (nDigits + 1), 1.0f);
