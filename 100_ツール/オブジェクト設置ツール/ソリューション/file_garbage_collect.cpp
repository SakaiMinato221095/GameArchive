...
//-===============================================
//-
//-	ゴミ回収のファイル処理[file_garbage_collect.cpp]
//- Author Sakai Minato
//-
//-===============================================
...
// ゴミ回収のテキストのコンスト定義
const char *pTxtGarbageCollect[] =
{
	"data\\TXT\\Object\\GarbageCollect_000.txt"		// ゴミ回収_000のテキスト
};
...
//-	コンストラクタ
//-------------------------------------
CFileGarbageCollect::CFileGarbageCollect()
{
}
...
//- ゴミ回収ファイルの書き出し処理
//-------------------------------------
void CFileGarbageCollect::Save(CFileGarbageCollect::TXT txtType)
{
	//変数宣言
	FILE *pFile;	// ファイルのポインタ

	// ファイルの情報を代入
	pFile = fopen(pTxtGarbageCollect[txtType], "w");
	...
	if (pFile != NULL)
	{
		...
		{
			// 変数宣言（指定の敵のポインタ取得）
			CGarbageCollect *pGarbageCollect = CManagerObject::GetGarbageCollect(nCount);

			// 敵の有無を判定
			if (pGarbageCollect != NULL)
			{
				// 変数宣言（敵の情報取得）
				int nModelNldx = pGarbageCollect->GetModel();			// 使用モデル番号
				D3DXVECTOR3 pos = pGarbageCollect->m_dataPos.Get();	// 位置
				D3DXVECTOR3 rot = pGarbageCollect->m_dataRot.Get();	// 向き
...
//- ゴミ回収ファイルの読み込み処理
//-------------------------------------
void CFileGarbageCollect::Load(CFileGarbageCollect::TXT txtType)
{
	//変数宣言
	FILE *pFile;	// ファイルのポインタ

	// ファイルの情報を代入
	pFile = fopen(pTxtGarbageCollect[txtType], "r");
	...
	if (pFile != NULL)
	{
		...
		CGarbageCollect *pGarbageCollect = NULL;	// 通常敵のポインタ
		...
		while (true)
		{
			...
			if (strstr(aFileData, "MODEL") != NULL)
			{
				if (strstr(aFileData, "=") != NULL)
				{
					for (int nCount = 0; nCount < ENEMY_NORMAL_NUM_MAX; nCount++)
					{
						...
						{
							// 敵の生成処理
							pGarbageCollect = CGarbageCollect::Create((CGarbageCollect::MODEL)0);
							...
							// モデル番号の取得
							int nModelNldx = CGarbageCollect::m_nDataModelNldx[nValue].Get();

							// モデル割当
							pGarbageCollect->BindModel(nModelNldx, (CGarbageCollect::MODEL)nValue);
			...
			if (strstr(aFileData, "POS") != NULL)
			{
				...
				if (strstr(aFileData, "=") != NULL)
				{
					...
					// 位置情報を代入
					pGarbageCollect->m_dataPos.Set(D3DXVECTOR3(
						fPosX,
						fPosY,
						fPosZ));
				}
			...
			if (strstr(aFileData, "ROT") != NULL)
			{
				...
				if (strstr(aFileData, "=") != NULL)
				{
					...
					// 向きを代入
					pGarbageCollect->m_dataRot.Set(D3DXVECTOR3(
						fRotX,
						fRotY,
						fRotZ));
				}
