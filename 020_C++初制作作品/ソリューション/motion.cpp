...
{
	"data\\TXT\\Motion\\PlayerSearch.txt",	// 探索プレイヤーのモーションファイル
	"data\\TXT\\Motion\\PlayerBattle.txt",	// 戦闘プレイヤーのモーションファイル
	"data\\TXT\\Motion\\Sample.txt",		// サンプルのモーションファイル
};
...
void CMotion::Set(int nType)
{
	...
	if (m_bMotionUse == false)
	{
		...
		{
			// キーの初期値で位置を設定
			m_ppModel[nCutModel]->m_dataMotionPos.Set(D3DXVECTOR3(
			...
			// キーの初期値で向きを設定
			m_ppModel[nCutModel]->m_dataMotionRot.Set(D3DXVECTOR3(
	...
	else
	{
		...
		for (int nCutModel = 0; nCutModel < m_nNumModel; nCutModel++)
		{
			D3DXVECTOR3 motionPos = m_ppModel[nCutModel]->m_dataMotionPos.Get();
			D3DXVECTOR3 motionRot = m_ppModel[nCutModel]->m_dataMotionRot.Get();

			m_dataMotionPosOld[nCutModel].Set(motionPos);
			m_dataMotionRotOld[nCutModel].Set(motionRot);
		}
	}
}
...
//- モーションのモデル設定処理
//-------------------------------------
void CMotion::SetModel(CObjectModel **ppModel, int nNumModel)
{
...
void CMotion::UpdateNormal(void)
{
	...
	for (int nCutModel = 0; nCutModel < m_nNumModel; nCutModel++)
	{
		...
		// 時間倍率の計算
		D3DXVECTOR3 motionPosTime = pManagerTime->CalRate(D3DXVECTOR3(
			(fDiffPosX * ((float)m_nCounter / (float)m_aInfo[m_nType].aKeyInfo[m_nKey].nFrame)),
			(fDiffPosY * ((float)m_nCounter / (float)m_aInfo[m_nType].aKeyInfo[m_nKey].nFrame)),
			(fDiffPosZ * ((float)m_nCounter / (float)m_aInfo[m_nType].aKeyInfo[m_nKey].nFrame))));

		// 目的の位置を設定
		m_ppModel[nCutModel]->m_dataMotionPos.Set(D3DXVECTOR3(
			(fPosX + motionPosTime.x),
			(fPosY + motionPosTime.y),
			(fPosZ + motionPosTime.z)));

		// 時間倍率の計算
		D3DXVECTOR3 motionRotTime = pManagerTime->CalRate(D3DXVECTOR3(
			(fDiffRotX * ((float)m_nCounter / (float)m_aInfo[m_nType].aKeyInfo[m_nKey].nFrame)),
			(fDiffRotY * ((float)m_nCounter / (float)m_aInfo[m_nType].aKeyInfo[m_nKey].nFrame)),
			(fDiffRotZ * ((float)m_nCounter / (float)m_aInfo[m_nType].aKeyInfo[m_nKey].nFrame))));

		// 目的の向きを設定
		m_ppModel[nCutModel]->m_dataMotionRot.Set(D3DXVECTOR3(
			(fRotX + motionRotTime.x),
			(fRotY + motionRotTime.y),
			(fRotZ + motionRotTime.z)));
	}
...
void CMotion::UpdateChangeBlend(void)
{
	...
	{
		// 前回の情報を取得
		D3DXVECTOR3 motionPosOld = m_dataMotionPosOld[nCutModel].Get();
		D3DXVECTOR3 motionRotOld = m_dataMotionRotOld[nCutModel].Get();
		...
		// 時間倍率の計算
		D3DXVECTOR3 motionPosTime = pManagerTime->CalRate(D3DXVECTOR3(
			(fDiffPosX * ((float)m_nCounter / (float)nFrame)),
			(fDiffPosY * ((float)m_nCounter / (float)nFrame)),
			(fDiffPosZ * ((float)m_nCounter / (float)nFrame))));
		...
		// 時間倍率の計算
		D3DXVECTOR3 motionRotTime = pManagerTime->CalRate(D3DXVECTOR3(
			(fDiffRotX * ((float)m_nCounter / (float)nFrame)),
			(fDiffRotY * ((float)m_nCounter / (float)nFrame)),
			(fDiffRotZ * ((float)m_nCounter / (float)nFrame))));
