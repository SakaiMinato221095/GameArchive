...
{
	"data\\TXT\\Model\\PlayerSearch.txt",	// 探索プレイヤーの階層構造モデルファイル
	"data\\TXT\\Model\\PlayerBattle.txt",	// 戦闘プレイヤーの階層構造モデルファイル
...
CDataInt CObjectModel::m_nDataModelNldx[MODEL_TYPE_MAX][MODEL_PARTS_MAX] = {};
...
HRESULT CObjectModel::Load(void)
{
	...
	for (int nCutModel = 0; nCutModel < MODEL_TYPE_MAX; nCutModel++)
	{
		...
		for (int nCntParts = 0; nCntParts < m_model[nCutModel].nPartsNum; nCntParts++)
		{
			...
			// モデル番号を設定
			m_nDataModelNldx[nCutModel][nCntParts].Set(nModelNldx);
		}
	}
...
HRESULT CObjectModel::Init(MODEL_TYPE modelType , int nCount)
{
	...
	// モデル番号を取得
	int nModelNldx = m_nDataModelNldx[modelType][nCount].Get();
...
void CObjectModel::Draw(void)
{
	...
	// モデル番号を取得
	int m_nModelNldx = m_nDataModelNldx[m_modelType][m_nPartsNum].Get();

	// モデル情報を取得
	CManagerModel::Model model = pManagerModel->GetAddress(m_nModelNldx);
...
CObjectModel *CObjectModel::Create(MODEL_TYPE modelType, int nCount)
{
	...
	CObjectModel *pObjectX = DBG_NEW CObjectModel;
