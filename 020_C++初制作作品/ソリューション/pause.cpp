...
HRESULT CPause::Init(void)
{
	...
	{
		// オブジェクト管理の生成
		m_apObj2dNone[TYPE_BG] = CObj2dNone::Create(CObj2dNone::TEX_PAUSE_BG);
	...
	for (int nCutPause = TYPE_GAME; nCutPause < TYPE_TITLE + 1; nCutPause++)
	{
		...
		if (m_apObj2dNone[nCutPause] == NULL)
		{
			...
			{
				// オブジェクト管理の生成
				m_apObj2dNone[nCutPause] = CObj2dNone::Create(CObj2dNone::TEX_PAUSE_GAME);
			}
			else if (nCutPause == TYPE_RETRY)
			{
				// オブジェクト管理の生成
				m_apObj2dNone[nCutPause] = CObj2dNone::Create(CObj2dNone::TEX_PAUSE_RETRY);
			}
			else if (nCutPause == TYPE_TITLE)
			{
				// オブジェクト管理の生成
				m_apObj2dNone[nCutPause] = CObj2dNone::Create(CObj2dNone::TEX_PAUSE_TITLE);
			}
	...
	{
		// オブジェクト管理の生成
		m_apObj2dNone[TYPE_CURSOR] = CObj2dNone::Create(CObj2dNone::TEX_PAUSE_CURSOR);
...
void CPause::Update(void)
{
	...
	if (pInputKeyboard->GetTrigger(DIK_W) != NULL)
	{
		...
		m_apObj2dNone[TYPE_CURSOR]->Set(
			D3DXVECTOR3(MENU_POS_X - 250.0f, MENU_POS_Y + (MENU_INTERVAL_Y * m_typeSelect), 0.0f),
			D3DXVECTOR3(30.0f, 30.0f, 0.0f));
	}
	...
	if (pInputKeyboard->GetTrigger(DIK_SPACE) != NULL)
	{
		m_bDataOk.Set(true);
	}
}
